...
@implementation CSStorePacketDetailView
...
- (id)initWithFrame:(CGRect)frame andNumberPacket:(int)numberPacket {
    ...
    if (self) {
        ...
        // add array item
        NSMutableArray *imageItem  = [[NSMutableArray alloc]initWithArray:[[CSGameManager sharedGameManager] pathImagesForPacketIndex:numberPacket + 1]];
...
- (void)loadPage:(int)pageIndex {
	...
	if((NSNull*)[pageViews objectAtIndex:pageIndex] == [NSNull null]) {
		...
		CSPacketDetailView* itemView = [[CSPacketDetailView alloc] initWithFrame:frame packets:[packets subarrayWithRange:range]];
